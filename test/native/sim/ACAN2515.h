...
extern uint8_t sim_pack_eflg[2];       // MCP2515 EFLG register, per pack
extern uint16_t sim_pack_rx_peak[2];   // driver receive-buffer high-water mark, per pack
class ACAN2515 { int port;
 public: ACAN2515(uint8_t cs, SPIClass&, uint8_t) : port(sim_port_for_cs(cs)) {}
  ...
  bool tryToSend(const CANMessage& m) { if (sim_pack_tx_fails) return false; SimFrame f; f.id=m.id; f.len=m.len;
      ...
      f.sentAtMs = sim_now_us / 1000;
      if(port>=0&&port<2) { sim_pack_tx[port].push_back(f); }
      return true; }
  ...
  void poll() {}
  // MCP2515 EFLG: bit 7 RX1OVR, 6 RX0OVR, 5 TXBO, 4 TXEP, 3 RXEP
  uint8_t errorFlagRegister() { return (port>=0&&port<2) ? sim_pack_eflg[port] : 0; }
  uint16_t receiveBufferSize() const { return 32; }
  uint16_t receiveBufferPeakCount() const { return (port>=0&&port<2) ? sim_pack_rx_peak[port] : 0; } };
